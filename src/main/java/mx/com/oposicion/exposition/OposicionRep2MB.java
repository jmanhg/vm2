...
@Controller
@Scope(value = "view")
public class OposicionRep2MB implements Serializable {
...
//          expireUserSessions(this.user.getUsername());
...
public void expireUserSessions(String username) {
    ...
    public void searchRegistroF(int op){
        ...
        if(loginMB.getAccessRol("PERFIL_ALL_SEARCH")){
            if(this.idUserSearch==0){
                msgControler.addInfo("Información", "Debe seleccionar un usuario" );
                return;
            }else{
                g.setPagina(this.idUserSearch);
            }
        }else{
            g.setPagina(this.user.getIdUsuario());
        }
    ...
    public void searchLimpia(){
        ...
        oposicionDataModel=null;
        this.numRegsBusqueda=0;
        this.numRegs=0;
        this.message="";
        msgControler.addInfo("Información", "Se limpio la tabla" );
    }
     public void oposicionLimpia(){
        oposicionDataModel=null;
        this.numRegs=0;
        this.message="";
        msgControler.addInfo("Información", "Se limpio la tabla de Resultados" );
    }
    ...
    public void handleFileUpload1(FileUploadEvent event) {
        this.tipoBusqueda=2;
        String path = OposicionRep2MB.class.getResource("OposicionRep2MB.class").getPath();
...
//               if(sinCeros) borraFonetic(1);
    ...
    public void searchFonetico() {
        ...
        int numGac=0;
            ...
            oposicionResultados= new ArrayList<OposicionData>();
            int numLine=0;
            numGac=gacetadataDataModel.getRowCount();
        ...
        msgControler.addInfo("Aviso", "Registros cargados: " + numLine + " de "+numGac+", se generará el Fonetico");
        System.out.println("Registros cargados: " + numLine + " de "+numGac+", se generará el Fonetico");
        BusquedaFonetica busquedaFonetica=new BusquedaFonetica();
         try {
             
             String strWhere="";
             if(this.fechaPub1!=null && this.fechaPub2!=null && this.fechaPub1.before(fechaPub2)){
                strWhere=" and F_Registro between '"+parseDate(this.fechaPub1,"yyyy/MM/dd")+"' and '"+parseDate(this.fechaPub2,"yyyy/MM/dd")+"' ";
             }
             if((this.fechaPub1!=null && this.fechaPub2==null) || (this.fechaPub1!=null && this.fechaPub1.equals(fechaPub2))){
                strWhere=" and F_Registro = '"+parseDate(this.fechaPub1,"yyyy/MM/dd")+"' ";
             }
             double progressInc=1d;
             if(oposicionDat.size()>0){
                progressInc=(double)100d/oposicionDat.size();
             }
             ...
             for (OposicionData oposicionData : oposicionDat) {
                progress+=progressInc;
                message = "Analizando reg["+(i++)+"/"+numGac+"] "+oposicionData.getDenominacion()+"; clase:"+oposicionData.getClases();
                System.out.println(message);
                 List<Gacetadata> fonetico = busquedaFonetica.BuscaFonetico(getDenominacionF(oposicionData.getDenominacion(),105), oposicionData.getClases(), oposicionData.getNumResultados(), "", "",strWhere, transactionManager.getDataSource().getConnection(),0);
                 List<Gacetadata> fonTmp= new ArrayList<Gacetadata>();
                 for(Gacetadata g :fonetico){
                     if(g.getCoeficiente()>=this.coeficiente){
                         fonTmp.add(g);
...
//               if(sinCeros) borraFonetic(1);
               ...
               message = "Búsqueda Fonética - Se encontraron "+oposicionResultados.size()+" registros en ["+String.format("%02d min, %02d seg", 
                                                                   ...
                                                                   )+"]; de un total de"+numGac+" registros";
               System.out.println(message);
               msgControler.addInfo("Información", message );
        } catch (Exception ex) {
                msgControler.addInfo("Información", "Ocurrio un erro al generar el fonetico:"+ex.getMessage() );
                System.out.println("SEVERO->Ocurrio un erro al generar el fonetico:"+ex.getMessage() );
        }
            this.stop=true;
    ...
    public void searchFigurativo(int bClase) {
        ...
        msgControler.addInfo("Aviso", "Registros cargados: " + numLine + " de "+numGac+", se generará el Figurativo");
        System.out.println("Registros cargados: " + numLine + " de "+numGac+", se generará el Figurativo");
...
//                            g.setImagen(g1.getImagen());
                            g.setTipoGaceta(vienaStyle(gacetadataVienaService.findCodVienaBySolicitudV(oposicionData.getSolicitud()),gacetadataVienaService.findCodVienaBySolicitud(g.getSolicitud())));
                            figTmp.add(g);
                        }
                    }
                 }else{
...
//                });
               ...
               long fin = System.currentTimeMillis();
               long total=fin-inicio;
               message = "Búsqueda Figurativa - Se encontraron "+oposicionResultados.size()+" registros en ["+String.format("%02d min, %02d seg", 
                                                                   TimeUnit.MILLISECONDS.toMinutes(total),
                                                                   TimeUnit.MILLISECONDS.toSeconds(total) - 
                                                                   TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(total))
                                                                   )+"]; de un total de"+numGac+" registros";
               ...
               msgControler.addInfo("Información", message );
                ...
                System.out.println("SEVERO->Ocurrio un erro al generar el figurativo:"+ex.getMessage() );
            }
            this.stop=true;
...
public static boolean isInteger(String s) {
 ...
 public byte[] generateDocument(String basePath, OposicionData od ,List<fonetic.Gacetadata> gacetadata) throws Exception {
        ...
        System.out.println("Se ha generado el documento PDF del Reporte Fonético de la solicitud ["+od.getSolicitud()+"]");
 ...
 public byte[] generateDocumentF(String basePath, OposicionData od ,List<mx.com.oposicion.model.Gacetadata> gacetadata,int clase) throws Exception {
        ...
        System.out.println("Se ha generado el documento PDF del Reporte Figurativo de la solicitud ["+od.getSolicitud()+"]");
        return streamReporteGenerado.toByteArray();
    }
 ...
 public byte[] generateDocumentF2(String basePath, OposicionData od ,List<mx.com.oposicion.model.Gacetadata> gacetadata,int clase) throws Exception {
        ...
        System.out.println("Se ha generado el documento PDF del Reporte2 Figurativo de la solicitud ["+od.getSolicitud()+"]");
        return streamReporteGenerado.toByteArray();
    }
            ...
            for (ReporteDatos r : reportes) {
                ...
                if(od.getTipo()==1){
                   ...
                   fig.add(new mx.com.oposicion.model.Gacetadata(rf.getIdRegistro(), rf.getTipo(), rf.getSubtipo(), rf.getRegistro(),
                           rf.getSolicitud(), rf.getDenominacion(), rf.getTitular(), rf.getGaceta(), rf.getOcurrencia(),
                           rf.getClase(), "", "", "","" , null, null, null,
                           rf.getViena(), rf.getApoderado(), "", "", "", "", null));
                   }
                }else{
                ...
                if (this.tipoBusqueda == 1) {
                    ...
                    for (mx.com.oposicion.model.Gacetadata gacetaTmp : fiDataModel) {
                        ReporteFonetico rf = new ReporteFonetico(0, reporteDatos.getIdReg(), gacetaTmp.getIdRegistro(),
                                ...
                                gacetaTmp.getTitulo(), gacetaTmp.getClase(), gacetaTmp.getTitular(), gacetaTmp.getApoderado(), gacetaTmp.getGaceta(),
                                gacetaTmp.getTipoGaceta(), 0.0, gacetaTmp.getPagina());
...
//    }
    ...
    private String getDenominacionF(String denominacion, int longitud) {
        String ret = "";
        if (denominacion == null || denominacion.length() == 0) {
            ret = "";
        } else if (denominacion.length() < longitud) {
            ret = denominacion.substring(0, denominacion.length());
        } else {
            ret = denominacion.substring(0, longitud);
